...
## Phase 1: Setup
- [X] T001 Create the `examples/publish-artifacts/` directory structure.

## Phase 2: User Story 1 — Publish to a repository from the CLI (P1) 🎯 MVP
- [X] T002 [US1] Author `examples/publish-artifacts/publish-artifacts.yaml` — native descriptor: a `build`
...
- [X] T003 [US1] Create `examples/publish-artifacts/sample-lib/` (standalone `java-library` + `maven-publish`
  build) that publishes to `$PUBLISH_REPO_URL` with optional `$PUBLISH_REPO_USER/PASSWORD` creds.
- [X] T004 [US1] Verify end-to-end: point `PROJECT_DIR` at `sample-lib`, `PUBLISH_REPO_URL` at a
  `file://` temp repo, run via the `kontinuance` CLI; assert the JAR + POM land in the repo and the run is Success.
- [X] T005 [US1] Verify fail-fast + masking: run with a missing credential secret (fail, no upload); confirm
  no secret value appears in run output.

## Phase 3: User Story 2 — Understandable & adaptable quickstart (P2)
- [X] T006 [US2] Write `examples/publish-artifacts/README.md`: what it does, the required secrets, how to run
