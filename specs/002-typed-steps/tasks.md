...
- [ ] T001 Confirm feature 001's seam is present: `StepDefinition` (sealed),
...
- [ ] T002 [P] Create the `steps/` package skeletons under `model/`, `execution/`,
...
- [ ] T003 Add a shared `descriptor` rule: a step declares exactly one of
...
- [ ] T004 [P] Establish the Konstellation builder scaffolding for typed steps under
...
- [ ] T005 [P] [US1] `GradleStep` model + validation test (`tasks` non-empty) in
  `model/steps/GradleStepTest.kt`.
- [ ] T006 [P] [US1] `GradleStepExecutor` argv-construction test in
...
- [ ] T007 [P] [US1] DSL + descriptor equivalence test
  (`dsl/steps/GradleStepDslTest.kt`): YAML `gradle:` and `gradleStep { }` → equal model.

### Implementation

- [ ] T008 [US1] Implement `model/steps/GradleStep.kt` (StepDefinition subtype).
- [ ] T009 [US1] Implement `execution/steps/GradleStepExecutor.kt` (`StepExecutor`):
  build argv, run via `StepContext`, map exit code; register into the 001 registry.
- [ ] T010 [US1] Implement the `gradleStep` Konstellation builder (`dsl/steps/`) and the
...
- [ ] T011 [P] [US2] `DockerStep` model test (RUN requires image+command; BUILD
  requires context) in `model/steps/DockerStepTest.kt`.
- [ ] T012 [P] [US2] `DockerStepExecutor` argv test in
...
- [ ] T013 [P] [US2] DSL + descriptor equivalence test for docker.

### Implementation

- [ ] T014 [US2] Implement `model/steps/DockerStep.kt` (+ `DockerMode`).
- [ ] T015 [US2] Implement `execution/steps/DockerStepExecutor.kt`; register it.
- [ ] T016 [US2] Implement the `dockerStep` builder + descriptor `docker:` mapping.
...
- [ ] T017 [P] [US3] `NpmStep` model test (SCRIPT requires script) in
  `model/steps/NpmStepTest.kt`.
- [ ] T018 [P] [US3] `NpmStepExecutor` argv test: SCRIPT → `npm run <script>`;
  INSTALL → `npm ci`/`npm install`; missing `npm` ⇒ FAILED. Real-`npm` path gated.
- [ ] T019 [P] [US3] DSL + descriptor equivalence test for npm.

### Implementation

- [ ] T020 [US3] Implement `model/steps/NpmStep.kt` (+ `NpmMode`).
- [ ] T021 [US3] Implement `execution/steps/NpmStepExecutor.kt`; register it.
- [ ] T022 [US3] Implement the `npmStep` builder + descriptor `npm:` mapping.
...
- [ ] T023 [P] Cross-type test: a mixed-step pipeline (run + gradle + docker + npm)
  executes correctly; a typed step and an equivalent `run` step show identical
  isolation + masking (SC-005).
- [ ] T024 Update `docs/diagrams/class-diagram.md` to show the three new subtypes +
  executors plugged into the seam.
- [ ] T025 Verify gates: `./gradlew build` (detekt zero violations, Kover passes —
  exemptions only via `@ExcludeFromCoverage` w/ justification); confirm **no**
  feature-001 engine-loop code changed (SC-003).
- [ ] T026 Run `quickstart.md` end-to-end and fix any drift.
