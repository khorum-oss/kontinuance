...
- **Unknown**: "latest Konstellation meta-DSL" — current pins are
  `konstellation-meta-dsl = 1.0.1` and `konstellation-dsl = 1.0.2`.
- **Decision**: Upgrade both to the newest releases published to the Khorum
  Reliquary repository (`https://open-reliquary.nyc3.cdn.digitaloceanspaces.com`)
  before implementation. The exact target version MUST be confirmed against that
  repository at implementation time (no network access during planning).
- **Rationale**: User explicitly requested the most recent Konstellation DSL. The
  hybrid DSL (Option C) is built on the meta-DSL, so it should track the latest.
- **Alternatives**: Stay on 1.0.1 — rejected per user direction.
- **Follow-up**: Bump in `gradle/libs.versions.toml`; regenerate/extend
  `gradle/verification-metadata.xml` (Constitution Principle V).
...
- Confirm and pin the newest Konstellation meta-DSL/dsl versions (R1).
