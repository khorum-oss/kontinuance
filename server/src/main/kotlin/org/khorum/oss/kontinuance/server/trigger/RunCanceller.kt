 ...
 * [RunLauncher]), so [PipelineEngine.cancel] can address it: the engine terminates the in-flight step and
 * the run ends `Cancelled`, which [RunLauncher] then records — so the UI sees the flip (instantly under the
