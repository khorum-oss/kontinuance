 ...
 * [kotlinx.coroutines.Dispatchers.Unconfined] scope so each background run executes inline (the paused/terminal record is
...
class RunApprovalsTest {
    ...
    private class Host(store: RunStore, descriptor: Path) {
        ...
        val launcher = RunLauncher(
            store,
            PipelineEngine.default({ }, approvalGate = gate),
