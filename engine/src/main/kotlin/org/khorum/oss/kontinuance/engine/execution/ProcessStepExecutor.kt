...
/**
 * The shared `ProcessBuilder`-based [StepExecutor] machinery: every step type that runs an external
 * command (shell, `gradle`, `docker`, `npm`, …) launches it exactly the same way, so they all
 * inherit identical isolation, timeout, masking, and status behavior (SC-005). A concrete executor
 * supplies only [command] — the argv to launch — and [supports]; this base owns everything else.
 *
 * Behavior (identical to the v0 [RunStepExecutor]):
 * - launches [command] inside the isolated working directory with the scoped environment supplied
 *   on the [StepContext] (the inherited process environment is cleared);
 ...
 * - a launch failure — including a **missing tool binary** — becomes a [PipelineStatus.Failed] step
...
abstract class ProcessStepExecutor(
    ...
    /**
     * The full argv to launch for this step, e.g. `["/bin/sh", "-c", "…"]` or
     * `["./gradlew", "build"]`. Pure: it must not start the process, only describe it. [workingDir]
     * is the step's isolated directory, provided so an executor can resolve tool wrappers relative
     * to it (e.g. a `./gradlew` present in a checked-out project).
     */
    protected abstract fun command(step: Step, workingDir: Path): List<String>
    ...
    override suspend fun execute(context: StepContext): StepRun = withContext(Dispatchers.IO) {
        ...
        val process = try {
            buildProcess(context, argv).start()
        } catch (e: IOException) {
    ...
    private fun buildProcess(context: StepContext, argv: List<String>): ProcessBuilder {
    ...
    companion object {
        /** Applied when a step declares no explicit timeout. */
        val DEFAULT_TIMEOUT: Duration = 1.hours
        private val CLEANUP_GRACE: Duration = 5.seconds
    }
}
